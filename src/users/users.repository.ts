...
export class UserRepository {
  ...
  async findAll(): Promise<object> {
    // how to correctly write what type of value function must return? With interface? or just type like object or string
    const result = await this.db.table(this.TABLE_NAME);
    return result.map((result) => {
      // iterate through result array to delete all hashes
  ...
  async findUserByEmail(email: string) {
    const result = await this.db.table(this.TABLE_NAME).where('email', email);
    return result[0];
  }
}
