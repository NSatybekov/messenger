...
import { FriendInterface, FriendRequestInterface, FriendRequestStatus } from './friends.entity';
...
export class FriendsRepository {
    ...
    async getFriendList(user_id: number){
        const message = await this.db.select('*').table(this.TABLE_NAME)
                    .where(function() {
                        this.where({user_id : user_id, friend_status: 'FRIEND'}).orWhere({friend_id : user_id, friend_status: 'FRIEND' })
                    })
        return message
    }
    ...
    async blockFriend(blocker_id : number, blocked_id: number, trx? : Knex.Transaction){
        const friend = await (trx || this.db).table(this.TABLE_NAME).insert({user_id: blocker_id, friend_id: blocked_id, friend_status: 'BLOCKED'})
                                                           .returning(['user_id', 'friend_id', 'friend_status', 'created_at'])
        return friend[0]
    }
