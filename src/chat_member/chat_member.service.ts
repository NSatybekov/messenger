...
import {
  ChatMemberCreateInterface,
  ChatMemberInterface,
  RoleEnum,
} from './chat_member.entity';
...
export class ChatMemberService {
  ...
  async addMemberToChatAttempt(
    iniciator: UserInterface,
    chat_id: number,
    user_id: number,
    role: RoleEnum,
  ) {
  ...
  async createChatMember(
    ...
    // when im inserting type its conflicting
    try {
      const memberData = {
  ...
  async createStandartChatMembers(
    chat_id,
    firstUser_id,
    secondUser_id,
  ): Promise<ChatMemberInterface[]> {
  ...
  async createMultipleChatMembers(
    membersData: ChatMemberCreateInterface[],
    trx?: Knex.Transaction,
  ) {
    return this.repository.createMultipleChatMembers(membersData, trx);
  }

  async updateUserRoleOrThrow(
    iniciator: UserInterface,
    chat_id: number,
    user_id: number,
    role: RoleEnum,
  ...
  async getChatMembers(user, chat_id): Promise<ChatMemberInterface[]> {
